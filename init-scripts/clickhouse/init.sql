...
SELECT
    ...
    toDate(parseDateTimeBestEffortOrNull(c9)) AS date_mutation,
    ...
    c22 AS section,
    c23 AS no_plan,
    toFloat32OrNull(replaceAll(c39, ',', '.')) AS surface_reelle_bati,
    c37 AS type_local,
    toFloat32OrNull(replaceAll(c43, ',', '.')) AS surface_terrain
FROM file(
    -- CHEMIN INTERNE DOCKER + EXTENSION .GZ
...
WHERE c9 != 'Date mutation' -- On ignore la ligne d'en-tête
SETTINGS format_csv_delimiter = ';'; -- Les fichiers DVF sont souvent en point-virgule